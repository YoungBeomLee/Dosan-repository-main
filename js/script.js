...
$(function () {
  ...
  function gnb() {
    ...
    $(".full_cover>ul>li").mouseover(function () {
      var i = $(this).index();
      $(".nav li").eq(current).find(".line").css("width", "100%");
    });
    $(".full_cover>ul>li").mouseleave(function () {
      var i = $(this).index();
      $(".nav li").eq(current).find(".line").css("width", "0%");
    });
  ...
  var obj = {
    ...
    pause: $(".pause_btn"),
    play: $(".play_btn"),
    remove: function (x) {x.removeClass("on");},
    add: function (x) {x.addClass("on");},
  };

  /*
	 시간마다 index(슬라이드) 계산함수,slide함수 호출함
	*/
  ...
  function slide() {
    ...
    slides
      ...
      .animate({ left: "0%" }, 1000, function () {
        ...
        //모든 애니메이션 요소 삭제
        obj.remove(obj.btn_ani);//0
        obj.remove(obj.logo);//0
        obj.remove(obj.text);//0
        obj.remove(obj.cbox);
        
        obj.add(obj.btn.eq(current).find(obj.btn_ani));//1
        obj.add(obj.btn.eq(current).find(".color"));
